            ...
            <div class="card">
              ...
              <div class="card-body">
                ...
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tbody>
                  <?php 
                  $qc = $conn->query("select * from tbl_order o left join tbl_mode_of_payment m on o.mode_of_payment_id = m.mode_of_payment_id inner join tbl_user u on u.user_id = o.user_id where o.order_id in (select order_id from tbl_product_order where product_id in (select product_id from tbl_product where store_id = '$store_id')) and o.status = 'Pending'") or die($conn->error);
                  while ($rc = $qc->fetch_assoc()){ 
                    ...
                    <tr>
                      ...
                      <?php if (($rc['mode_of_payment_id'] > 0 && $qp->num_rows != 0) || $rc['mode_of_payment_id'] == 0){ ?>
                       <button type="button" data-id="<?php echo $rc['order_id']; ?>" data-question="Are you sure you want to confirm payment for <strong>ODR-<?php echo str_pad($rc['store_id'], 4, "0", STR_PAD_LEFT).'-'.str_pad($rc['order_id'], 8, "0", STR_PAD_LEFT); ?></strong>?" data-id="<?php echo $rc['order_id']; ?>" data-toggle="modal" data-target="#confirmModal" class="btn btn-sm btn-success confirm" title="Confirma Payment"><i class="fa fa-peso-sign"></i></button>
                      <?php } else { ?>
                      <span class="btn btn-secondary btn-sm"><i class="fa fa-peso-sign"></i> </span>
                      <?php } ?>
                      <?php if ($rc['status'] == 'Pending' && $qp->num_rows == 0) { ?>
                      <button type="button" data-id="<?php echo $rc['order_id']; ?>" data-question="Are you sure you want to cancel <strong>ODR-<?php echo str_pad($rc['store_id'], 4, "0", STR_PAD_LEFT).'-'.str_pad($rc['order_id'], 8, "0", STR_PAD_LEFT); ?></strong>?"  data-id="<?php echo $rc['order_id']; ?>" data-toggle="modal" data-target="#confirmModal" class="btn btn-sm btn-danger cancel" title="Cancel Order"><i class="fa fa-trash"></i></button>
                      <?php } else { ?>
...
<script type="text/javascript">
  $(document).ready(function(){
    ...
    $(".confirm").click(function(){
          $("#confirm_title").html("Confirm Cancel");
          $("#modal_confirm").html($(this).data('question'));
          $("#btn_confirm").attr("href", 'order-payment-confirm.php?id=' + $(this).data('id'));
      });
